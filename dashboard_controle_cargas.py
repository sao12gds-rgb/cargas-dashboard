from future import annotationsfrom io import BytesIOimport pandas as pdimport plotly.express as pximport streamlit as stfrom datetime import datetime
...
st.set_page_config(page_title="Dashboard Controle de Cargas",page_icon="📦",layout="wide",initial_sidebar_state="expanded",)
...
def normalizar_awb(valor: object) -> str:"""Normaliza AWB para 8 dígitos"""if pd.isna(valor):return ""
...
def encontrar_coluna(df: pd.DataFrame, opcoes: list[str]) -> str | None:"""Encontra coluna de forma flexível"""colunas_lower = {col.lower().strip(): col for col in df.columns}
...
st.markdown('📁 Carregar Dados', unsafe_allow_html=True)

col1, col2 = st.columns(2)

with col1:st.info("1️⃣ Relatório do Sistema (CSV ou Excel)")arquivo_sistema = st.file_uploader("Selecione o arquivo",type=["xlsx", "xls", "csv"],key="sistema")

with col2:st.info("2️⃣ Planilha Consolidada (Excel com 4 abas)")arquivo_consolidado = st.file_uploader("Selecione o arquivo",type=["xlsx", "xls"],key="consolidado")
